 ...
 */
async function getEnvironmentFingerprint(databaseUrl) {
  try {
    ...
    try {
      const metadata = await tempPrisma.$queryRaw`
        SELECT fingerprint 
 ...
 */
function logAuditFailure(operation, mutationContext, error) {
 ...
 */
async function createMutationAuditLog(capability, databaseUrl, metadata) {
  ...
  try {
    ...
    logAuditFailure('create', mutationContext, error);
    return null;
  }
}
 ...
 */
async function updateMutationAuditLogStatus(
  auditLogId,
  databaseUrl,
  status,
  errorMessage,
  ...
  try {
    ...
    // Non-blocking: log failure but don't throw
    logAuditFailure('update', mutationContext, error);
  }
}
