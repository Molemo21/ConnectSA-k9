...
import { Loader2, FileText, MapPin, ArrowRight, ArrowLeft, Search, CalendarDays, Clock } from "lucide-react"
  ...
  return (
    <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
      ...
      <CardContent className="space-y-4 sm:space-y-6">
        ...
        {!isLoading && (
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 sm:space-y-6" noValidate>
              ...
              <div className="rounded-md border border-gray-200">
                ...
                <div className="max-h-56 overflow-auto">
                  {recentServiceIds.length > 0 && (
                    <div className="px-3 pt-2 pb-1 text-[11px] uppercase tracking-wide text-gray-500">Recent</div>
                  )}
                  {recentServiceIds.map((id) => {
                    const s = services?.find((x: any) => x.id === id)
                    if (!s) return null
                    return (
                      <button type="button" key={`recent-${id}`} onClick={() => handleFieldChange('serviceId', id)} className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${value.serviceId===id?'bg-indigo-50':''}`}>
                  ...
                  {filteredServices && filteredServices.length > 0 ? (
                    filteredServices.map((service: any) => (
                      <button type="button" key={service.id} onClick={() => handleFieldChange('serviceId', service.id)} className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${value.serviceId===service.id?'bg-indigo-50':''}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input type="date" name="date" value={value.date} onChange={(e) => handleFieldChange('date', e.target.value)} min={new Date().toISOString().split('T')[0]} required className="pl-9 h-10 sm:h-11 text-sm sm:text-base" aria-invalid={!!errors.date} />
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input type="time" name="time" value={value.time} onChange={(e) => handleFieldChange('time', e.target.value)} required className="pl-9 h-10 sm:h-11 text-sm sm:text-base" aria-invalid={!!errors.time} />
                </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {timeSlots.slice(0, 6).map((t) => (
                      <button
                        ...
                        className={`px-2 py-1 rounded-full text-xs border ${value.time===t?'bg-indigo-50 border-indigo-200 text-indigo-700':'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                      >
                        {t}
                      </button>
                    ))}
            ...
            <div className="space-y-2">
              ...
              <div className="relative flex gap-2">
                <div className="relative flex-1">
                  ...
                  <Input name="address" value={value.address} onChange={(e) => handleFieldChange('address', e.target.value)} placeholder="Enter your address" className="pl-10 h-10 sm:h-11 text-sm sm:text-base" required aria-invalid={!!errors.address} autoComplete="street-address" />
                </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 mb-1">
                {['Parking info','Gate/Access code','Pet on premises','Preferred time window'].map((chip) => (
                  <button type="button" key={chip} onClick={() => handleFieldChange('notes', `${(value.notes||'').trim()}${value.notes? '\n' : ''}${chip}: `)} className="px-2 py-1 rounded-full text-xs border border-gray-200 text-gray-600 hover:bg-gray-50">{chip}</button>
                ))}
              </div>
              <Textarea name="notes" value={value.notes} onChange={(e) => handleFieldChange('notes', e.target.value)} placeholder="Any specific requirements or details..." rows={3} className="text-sm sm:text-base min-h-[3rem] resize-none whitespace-pre-line" maxLength={NOTES_MAX} />
            ...
            <div className="flex items-center justify-between">
              ...
              <Button type="submit" disabled={submitting} className="hidden md:inline-flex h-11 sm:h-12 text-sm sm:text-base font-medium">
                {submitting ? (
                  <>
            ...
            <div className="md:hidden fixed left-0 right-0 bottom-20 px-4">
              <Button type="submit" disabled={!isValid || submitting} className="w-full h-12 text-base shadow-lg">
                {submitting ? (
                  <>
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                    Processing...
        ...
        {selectedService && (
          <div className="text-xs text-gray-500">
            Selected: <span className="font-medium text-gray-700">{selectedService.name}</span> • {selectedService.category}
          </div>
        )}
      </CardContent>
    </Card>
