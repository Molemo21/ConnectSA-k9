...
/**
 * Payment Flow Test - Final Corrected Version with Right Column Names
 */

const { PrismaClient } = require('@prisma/client');

async function testPaymentFlow() {
  console.log('��� Testing Complete Payment Flow (Column Names Fixed)...\n');
  ...
  try {
    ...
    // Test 2.5: Create provider_services relationship with correct column names
    console.log('\n2️⃣.5️⃣ Creating provider-service relationship...');
    const providerServiceResult = await prisma.$executeRaw`
      INSERT INTO provider_services (id, "providerId", "serviceId", "customRate")
      VALUES (gen_random_uuid()::text, ${testData.provider.id}, ${testData.service.id}, 1000)
    `;
    ...
    const providerService = await prisma.$queryRaw`
      ...
      WHERE "providerId" = ${testData.provider.id} AND "serviceId" = ${testData.service.id}
    `;
