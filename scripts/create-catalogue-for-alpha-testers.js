...
function generateCatalogueItem(provider, service, tier, index) {
  const serviceCategory = getServiceCategory(service.name);
  const hourlyRate = provider.hourlyRate || 150;
  const multiplier = pricingMultipliers[tier.toLowerCase()];
  ...
  const titleTemplate = getRandomElement(titleTemplates[serviceCategory] || titleTemplates.default);
