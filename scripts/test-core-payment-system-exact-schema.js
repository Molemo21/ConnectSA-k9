...
/**
 * Core Payment System Test - Exact Schema Match
 */

const { PrismaClient } = require('@prisma/client');

async function testCorePaymentSystem() {
  console.log('í·ª Testing Core Escrow Payment System (Exact Schema)...\n');
  ...
  try {
    ...
    // Test 3: Create test booking with EXACT required fields only
    ...
    testData.booking = await prisma.booking.create({
      data: {
        ...
        // Only using the fields that are actually required
      }
    });
    ...
    if (summary.escrow_amount === calculatedEscrow && summary.platform_fee === 100) {
      ...
      console.log(`   Total: ${summary.total_amount} NGN`);
    ...
    console.log('í³ Note: Using exact schema requirements only');
